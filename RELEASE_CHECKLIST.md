...
- [ ] Release 빌드에서 Debug 빌드의 프리미엄 저장값이 남아 있어도 프리미엄으로 판정되지 않는다.
- [ ] Release 빌드에서 legacy debug premium key(`is_premium`)가 남아 있어도 무료 상태로 표시된다.
- [ ] Release 빌드에서 debug premium key(`debug_is_premium`)가 남아 있어도 무료 상태로 표시된다.
- [ ] Billing 미연결 상태에서는 Release 빌드의 프리미엄 판정이 false임을 확인한다.
...
- [ ] 병합 복원은 성공 시 거래/계좌/반복 거래/skipped occurrence가 함께 반영된다.
- [ ] 병합 복원 실패 시 기존 거래/계좌/반복 거래 데이터가 유지된다.
- [ ] 병합 복원 후 중복 id 데이터가 추가되지 않는다.
- [ ] 병합 복원 후 반복 거래 sync가 실행되어 due scheduled 거래가 posted로 전환된다.
- [ ] 병합 복원 후 반복 occurrence가 중복 생성되지 않는다.
- [ ] 교체 복원은 성공 시 기존 데이터가 백업 데이터로 완전히 대체된다.
- [ ] 교체 복원 실패 시 기존 거래/계좌/반복 거래 데이터가 유지된다.
- [ ] 교체 복원 후 홈 월 정산, 전체보기, 계좌 계산 잔액, 반복 거래 목록이 복원 데이터 기준으로 갱신된다.
- [ ] 교체 복원 후 기존 영수증 이미지 파일 정리가 가능한 범위에서 실행된다.
- [ ] 전체 데이터 삭제는 거래/계좌/반복 거래/skipped occurrence를 한 번에 삭제한다.
- [ ] 전체 데이터 삭제 실패 시 일부 테이블만 삭제된 상태가 남지 않는다.
- [ ] 전체 데이터 삭제 후 홈/전체보기/계좌/반복 거래 화면이 빈 상태로 표시된다.
- [ ] 영수증 이미지 파일 삭제는 DB transaction 밖에서 처리되므로, 파일 삭제 실패 가능성을 release note/test note에 인지한다.
...
- [ ] 무료 상태에서 상단 배너가 표시된다.
- [ ] 프리미엄 상태에서 광고가 숨겨진다.
- [ ] Debug 빌드에서만 개발용 프리미엄 상태 전환이 가능하다.
- [ ] Release 빌드에서는 Debug 저장값과 무관하게 로컬 프리미엄이 활성화되지 않는다.
...
- [ ] JSON 병합/교체/전체삭제가 transaction 단위로 안전하게 동작한다.
- [ ] 월별/연도별 PDF 생성과 공유가 동작한다.
- [ ] 영수증 첨부/확대가 동작한다.
- [ ] 앱 재실행 후 데이터가 유지된다.
- [ ] Debug 도구가 노출되지 않는다.
- [ ] 개발용 프리미엄 토글이 노출되지 않는다.
- [ ] Debug 빌드에서 프리미엄을 켠 뒤 Release 빌드를 덮어 설치해도 무료 상태로 표시된다.
